...
function processPageContent(htmlContent, pageNum, pages, callback) {
    ...
    // 新结构: 每个视频卡片是一个 <a href=...> 包含 <video> 和 <img>
    const cardElements = doc.querySelectorAll("a[href*='/cn/']")

    const logEntry = {
        url: `${processUrl(inurl)}${pageNum}`,
        elementsFetched: cardElements.length
    }

    if (!downloadLog[name]) downloadLog[name] = []
    ...
    if (cardElements.length === 0) {
        ...
        if (!errorLogs[name]) errorLogs[name] = []
        errorLogs[name].push(errorEntry)
        console.log(`获取第 ${pageNum} 页失败。`)
        window.addToLog(`${name}${processUrl(inurl)}${pageNum}+获取失败 数量：` + cardElements.length, 'error')
    }

    cardElements.forEach(a => {
        let url = a.href
        let img = a.querySelector('img')?.getAttribute('data-src') || a.querySelector('img')?.src
        let videoUrl = a.querySelector('video')?.getAttribute('data-src')
        let title = a.querySelector('img')?.alt || a.getAttribute('alt') || ''
